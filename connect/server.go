...
func (s *Server) writePump(ch *Channel, c *Connect) {
	...
	for {
		...
		case message, ok := <-ch.broadcast:
			// write data dead time, link http timeout, default 10s
...
func (s *Server) readPump(ch *Channel, c *Connect) {
	defer func() {
		...
		if ch.Room == nil || ch.userId == 0 {
			logrus.Infof("roomId and usserId eq 0")
			ch.conn.Close()
			return
		}
	...
	ch.conn.SetWriteDeadline(time.Now().Add(s.Options.PongWait))
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				logrus.Errorf("readPump ReadMessage eerr:%s", err.Error())
				return
			}
		}
		if message == nil {
		...
		if err != nil {
			logrus.Errorf("s.operatot.Connect error %s", err.Error())
			return
		}
		if userId == 0 {
		...
		logrus.Infof("websockeet rpc call return userId:%d,RoomId:%d", userId, connReq.RoomId)
		b := s.Bucket(userId)

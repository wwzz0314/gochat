...
func (c *Connect) InitTcpserver() error {
	...
	for _, ipPort := range aTcpAddr {
		...
		if listener, err = net.ListenTCP("tcp", addr); err != nil {
			logrus.Errorf("net.ListenTCP(tcp, %s),err(%v)", ipPort, err)
			return err
		}
...
func (c *Connect) acceptTcp(listen *net.TCPListener) {
	var (
		conn *net.TCPConn
		err  error
		r    int
	...
	for {
		if conn, err = listen.AcceptTCP(); err != nil {
			logrus.Errorf("listen.Accpet(\"%s\")error(%v)", listen.Addr().String(), err)
			return
		}
...
func (c *Connect) readDataFromTcp(s *Server, ch *Channel) {
	defer func() {
		...
		logrus.Infof("exec diisConnectt ...")
		...
		if err := ch.connTcp.Close(); err != nil {
			logrus.Warnf("DisConnect closse tcp conn err :%s", err.Error())
		}
		return
	}()
	//scanner
	...
	for {
		...
		for scannerPackage.Scan() {
			...
			if err != nil {
				logrus.Errorf("scan tcp pakage err:%s", err.Error())
				break
			}
			// get a full package
			...
			logrus.Infof("json unmarshal,eaw tcp msg is:%v", rawTcpMsg)
...
func (c *Connect) writeDataToTcp(s *Server, ch *Channel) {
	...
	for {
		...
		case message, ok := <-ch.broadcast:
			...
			// send message
			logrus.Infof("send tcp msg to connL%s", pack.String())
